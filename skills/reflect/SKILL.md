---
name: reflect
description: "Self-learning loop for the personal vault. Reviews the current session for what worked / didn't / was corrected by the user, extracts durable lessons and new facts, and updates wiki/LESSONS.md (how to work with the user) and the vault (facts about the user) with provenance + outcome signals + corroboration + time-decay. Triggers on: reflect, /reflect, learn from this session, update lessons, what did you learn, self-learning."
---
...
- User says "reflect" / "/reflect" / "update lessons" / "what did you learn" (in any language).
...
| **Corrected** | user pushed back and redirected ("no, not like that", "I need precision") | `[SAID]` | `corrected` |
...
- **Format:** `` - `[PROVENANCE×N]` **lesson** — short why. · signal · YYYY-MM-DD ``
- **Corroboration gate:** a lesson enters **Trusted** only after **≥2 independent confirmations**. First observation → **Tentative**. On a repeat confirmation, bump the `×N` and the date, and promote if N≥2.
- **Time-decay / conflict:** if a fresh `corrected`/`dead_end` contradicts an existing lesson, move both readings to **Contested** and let recency win; if the old lesson is clearly dead, move it to **Archive** with a one-line note.
- **A `corrected` signal** also gets a line in **Corrections from the user** (the rule learned from the mistake).
...
- **One signal ≠ a rule.** Resist minting a "Trusted" lesson from a single interaction — that's how a model overfits to a one-off mood.
